...
function _multiply!{T<:Union{GenericAffExpr,GenericQuadExpr}}(ret::AbstractArray{T}, lhs::SparseMatrixCSC, rhs::AbstractArray)
    nzv = nonzeros(lhs)
    rv  = rowvals(lhs)
    for col ∈ 1:lhs.n
        for k ∈ 1:size(ret, 2)
...
# this computes lhs.'*rhs and places it in ret
function _multiplyt!{T<:Union{GenericAffExpr,GenericQuadExpr}}(ret::AbstractArray{T}, lhs::SparseMatrixCSC, rhs::AbstractArray)
    _multiply!(ret, transpose(lhs), rhs) # TODO fully implement
end

function _multiply!{T<:Union{GenericAffExpr,GenericQuadExpr}}(ret::AbstractArray{T}, lhs::AbstractMatrix, rhs::SparseMatrixCSC)
    rowval = rowvals(rhs)
    nzval  = nonzeros(rhs)
...
# this computes lhs.'*rhs and places it in ret
function _multiplyt!{T<:Union{GenericAffExpr,GenericQuadExpr}}(ret::AbstractArray{T}, lhs::AbstractMatrix, rhs::SparseMatrixCSC)
    rowval = rowvals(rhs)
    nzval  = nonzeros(rhs)
